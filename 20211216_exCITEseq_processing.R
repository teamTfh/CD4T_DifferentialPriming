...
setwd("~/OneDrive - NYU Langone Health/ECCITE-seq/Experiments/20211203_ECCITEseq_Subject9_Subject7")
#nCount = UMI
...
adt_assay_stim <- adt_stim[setdiff(rownames(x = adt_stim),c("PHI_Subject9_7moPostVax", "PHI_Subject9_1moPostBoost", "PHI_Subject7_7moPostVax", "PHI_Subject7_1moPostBoost", "CD45")), ]
adt_assay_unstim <- adt_unstim[setdiff(rownames(x = adt_unstim),c("PHI_Subject9_7moPostVax", "PHI_Subject9_1moPostBoost", "PHI_Subject7_7moPostVax", "PHI_Subject7_1moPostBoost", "CD45")), ]
...
HTO_assay_stim <- adt_stim[intersect(rownames(x = adt_stim),c("PHI_Subject9_7moPostVax", "PHI_Subject9_1moPostBoost", "PHI_Subject7_7moPostVax", "PHI_Subject7_1moPostBoost")), ]
HTO_assay_unstim <- adt_unstim[intersect(rownames(x = adt_unstim),c("PHI_Subject9_7moPostVax", "PHI_Subject9_1moPostBoost", "PHI_Subject7_7moPostVax", "PHI_Subject7_1moPostBoost")), ]
...
empty_droplets_stim <- subset(x = stim, subset = nCount_RNA <= 500)
empty_droplets_unstim <- subset(x = unstim, subset = nCount_RNA <= 500)
...
cells_stim <- subset(x = stim, subset = nCount_RNA > 500)
cells_unstim <- subset(x = unstim, subset = nCount_RNA > 500)
...
cells_stim$hash.ID <- factor(cells_stim$hash.ID, levels = c("Negative", "Subject7-7moPostVax", "Subject7-1moPostBoost",
                                                            "Subject9-7moPostVax", "Subject9-1moPostBoost", "Doublet"))
# Group cells based on the max HTO signal
Idents(cells_stim) <- "HTO_maxID"
RidgePlot(cells_stim, assay = "HTO", features = rownames(cells_stim[["HTO"]])[1:4], ncol = 4)
s <- FeatureScatter(cells_stim, feature1 = "Subject7-7moPostVax", feature2 = "Subject7-1moPostBoost", group.by="hash.ID") + labs(title = "stim") + NoLegend()
Idents(cells_stim) <- "hash.ID"
s.1 <- VlnPlot(cells_stim, features = "nCount_HTO", pt.size = 0.1, log = TRUE)  + labs(title = "stim") +  NoLegend() + 
  geom_hline(yintercept = 5.9)+ scale_y_continuous(limits = c(1,100000), trans = "log10")
s.2 <- RidgePlot(cells_stim, assay = "HTO", features = rownames(cells_stim[["HTO"]]), group.by="hash.ID", combine=TRUE, ncol = 1)

##Unstim
table(cells_unstim$HTO_classification.global)
cells_unstim$hash.ID <- factor(cells_unstim$hash.ID, levels = c("Negative", "Subject7-7moPostVax", "Subject7-1moPostBoost",
                                                              "Subject9-7moPostVax", "Subject9-1moPostBoost", "Doublet"))
# Group cells based on the max HTO signal
Idents(cells_unstim) <- "HTO_maxID"
RidgePlot(cells_unstim, assay = "HTO", features = rownames(cells_unstim[["HTO"]])[1:4], ncol = 4)
u <- FeatureScatter(cells_unstim, feature1 ="Subject7-7moPostVax", feature2 = "Subject7-1moPostBoost", group.by="hash.ID") + labs(title = "unstim") + NoLegend()
...
u.2 <- RidgePlot(cells_unstim, assay = "HTO", features = rownames(cells_stim[["HTO"]]), group.by="hash.ID", combine=TRUE, ncol = 1)

#FEATURE SCATTER PLOT OF HTOS 
s | u 
s.1| u.1 #violin plots w nCount_HTO
s.2 | u.2 #ridge plots
...
cells_stim <- subset(cells_stim, subset = nCount_HTO >= 6)
...
s <- FeatureScatter(cells_stim, feature1 = "Subject7-7moPostVax", feature2 = "Subject7-1moPostBoost", group.by="stim_ID") + labs(title = "stim")
...
stim_hash <- cells_stim@meta.data %>% 
  ...
  ggtitle("Stim NCells") + ylim(c(0,22000)) +
...
sce.stim <- scDblFinder(sce.stim, dbr = (0.004 * ncol(sce.stim)/1000), samples="HTO_maxID")
...
cells_stim$scDblFinder.class <- factor(cells_stim$scDblFinder.class, levels=c("doublet", "singlet"))
cells_unstim$scDblFinder.class <- factor(cells_unstim$scDblFinder.class, levels=c("doublet", "singlet"))

s.21 <- FeatureScatter(cells_stim, feature1 = "Subject7-7moPostVax", feature2 = "Subject7-1moPostBoost", group.by ="scDblFinder.class") + labs(title = "stim")
u.21 <- FeatureScatter(cells_unstim, feature1 = "Subject7-7moPostVax", feature2 = "Subject7-1moPostBoost", group.by="scDblFinder.class") + labs(title = "unstim")

s.21| u.21

s.48 <- FeatureScatter(cells_stim, feature1 = "Subject9-7moPostVax", feature2 = "Subject9-1moPostBoost", group.by ="scDblFinder.class") + labs(title = "stim")
u.48 <- FeatureScatter(cells_unstim, feature1 = "Subject9-7moPostVax", feature2 = "Subject9-1moPostBoost", group.by="scDblFinder.class") + labs(title = "unstim")

s.48  | u.48

cells_stim$HTO_maxID <- factor(cells_stim$HTO_maxID, levels=c("Subject7-7moPostVax","Subject7-1moPostBoost", "Subject9-7moPostVax", "Subject9-1moPostBoost"))
cells_unstim$HTO_maxID <- factor(cells_unstim$HTO_maxID, levels=c("Subject7-7moPostVax","Subject7-1moPostBoost", "Subject9-7moPostVax", "Subject9-1moPostBoost"))

Idents(cells_stim) <- "scDblFinder.class"
s.1 <- VlnPlot(cells_stim, features = "nCount_HTO", pt.size = 0.1, log = TRUE)  + labs(title = "stim") +  NoLegend() + 
  geom_hline(yintercept = 5.9)+ scale_y_continuous(limits = c(1,100000), trans = "log10")
...
VlnPlot(object = cells_stim, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3, group.by = "orig.ident" ) 
...
par(mfrow = c(1, 2))
FeatureScatter(object = cells_stim, feature1 = "nCount_RNA", feature2 = "percent.mito", group.by = "orig.ident" ) + geom_hline(yintercept = 0.08) + labs(title = "stim")
FeatureScatter(object = cells_stim, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", group.by = "scDblFinder.class") +  geom_hline(yintercept = c(400, 7000)) + labs(title = "stim")

#Do doublets have higher nCount and nFeature and negatives have lower nCount and nFeature?
Idents(cells_stim) <- "scDblFinder.class"
RidgePlot(cells_stim, features = c("nCount_RNA", "nFeature_RNA"), log = TRUE)
...
cells_stim <- subset(x = cells_stim, subset = nFeature_RNA > 400 & nFeature_RNA < 7000 & percent.mito >  -Inf & percent.mito < 0.08 ) 
...
DimPlot(cells_stim, label = TRUE, group.by = "HTO_maxID", pt.size = 1)
DimPlot(cells_stim, label = TRUE, pt.size = 1)
...
# We filter out cells that have unique gene counts (nFeature_RNA) over 4,000 or less than 200 Note that > and < are used to define a 'gate' 
# -Inf and Inf should be used if you don't want a lower or upper threshold.
cells_unstim <- subset(x = cells_unstim, subset = nFeature_RNA > 400 & nFeature_RNA < 4500 & percent.mito >  -Inf & percent.mito < 0.08 ) 
...
cells_stim@meta.data$HTO_classification.global
#CD19 vs CD3 gene expression before filtering out doublets
stim_hash <- FeatureScatter(cells_stim, feature1 = "CD3E", feature2 = "CD19", group.by = "scDblFinder.class") + NoLegend() + labs(title="stim") #+ geom_vline(xintercept = 0.75) +  geom_hline(yintercept = 0.75)
...
stim_hash <- FeatureScatter(stim_singlet, feature1 = "CD3E", feature2 = "CD19", group.by = "HTO_maxID") + geom_vline(xintercept = 0.25) +  geom_hline(yintercept = 0.25) + labs(title = "stim") + NoLegend()
...
stim_singlet.2@meta.data <- stim_singlet.2@meta.data %>% separate(HTO_maxID, c("subject", "subject1", "timepoint"), sep = "-", remove = FALSE)
stim_singlet.2@meta.data <- stim_singlet.2@meta.data %>% unite("subject", subject:subject1, remove=TRUE, sep = "-")
stim_singlet.2@meta.data <- stim_singlet.2@meta.data  %>%
  mutate(PriorCOVID = case_when(
    endsWith(subject, "Subject9") ~ "yes",
    endsWith(subject, "Subject7") ~ "no"
  ))
...
saveRDS(stim_singlet.2, file = "20211203_stim.rds")
saveRDS(uns_singlet.2, file = "20211203_unstim.rds")
